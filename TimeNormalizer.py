...
class TimeNormalizer:
    ...
    def _filter(self, input_query):
        ...
        input_query = input_query.replace(u'中旬', u'15号')
        input_query = input_query.replace(u'傍晚', u'午后')
        input_query = input_query.replace(u'大年', '')
        input_query = input_query.replace(u'五一', u'劳动节')
        input_query = input_query.replace(u'白天', u'早上')
        input_query = input_query.replace(u'：', u':')
        return input_query

    def init(self):
        fpath = os.path.dirname(__file__) + '/resource/reg.pkl'
        with codecs.open(os.path.dirname(__file__) + '/resource/regex.txt', 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        with codecs.open(os.path.dirname(__file__) + '/resource/holi_solar.json', 'r', encoding='utf-8') as f:
            holi_solar = json.load(f)
        with codecs.open(os.path.dirname(__file__) + '/resource/holi_lunar.json', 'r', encoding='utf-8') as f:
